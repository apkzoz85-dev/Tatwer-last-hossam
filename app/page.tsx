  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({ access_key: WEB3_KEY, botcheck: "", name: form.name, phone: form.phone, project: "SALT North Coast — تطوير مصر", subject: "Lead — SALT سولت رأس الحكمة", cc: CC_EMAIL }),
      })
...
export default function Home() {
  ...
  const submitPopup = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("https://api.web3forms.com/submit", {
        method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json" },
        body: JSON.stringify({ access_key: WEB3_KEY, botcheck: "", name: popupForm.name, phone: popupForm.phone, subject: "Lead — SALT سولت رأس الحكمة (Popup)", cc: CC_EMAIL }),
      })
      ...
      <section style={{ position: "relative", height: "100vh", overflow: "hidden" }}>
        ...
        <div className="hero-inner" style={{ position: "absolute", bottom: 0, left: 0, right: 0, zIndex: 10, padding: "0 48px 72px", maxWidth: 800 }}>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            ...
            <a href={`${WA}?text=${encodeURIComponent("مرحباً، أنا مهتم بمشروع SALT North Coast من تطوير مصر وأريد معرفة المزيد عن الوحدات المتاحة وأنظمة السداد.")}`} target="_blank" rel="noopener noreferrer" onClick={trackWhatsApp}
      ...
      <section id="contact">
        <div className="contact-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", minHeight: "60vh" }}>
          <div className="c-left" style={{ background: "#FF4713", padding: "56px 48px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
            ...
            <div style={{ display: "flex", gap: 12 }}>
              <a href={`${WA}?text=${encodeURIComponent("مرحباً، أنا مهتم بمشروع SALT North Coast من تطوير مصر وأريد معرفة المزيد")}`} target="_blank" rel="noopener noreferrer" onClick={trackWhatsApp}
            ...
            <div style={{ padding: "24px 28px" }}>
              {popupSent ? (
                ...
                <form onSubmit={submitPopup}>
                  ...
                  <a href={`${WA}?text=${encodeURIComponent("مرحباً، أنا مهتم بمشروع SALT North Coast من تطوير مصر")}`} target="_blank" rel="noopener noreferrer" onClick={trackWhatsApp}
